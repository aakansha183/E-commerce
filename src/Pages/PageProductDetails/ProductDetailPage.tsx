...
const ProductDetail: React.FC = () => {
  ...
  const handleAddToCart = () => {
    ...
    const isItemInCart = cartItems.find((item) => item.id === product.id && item.size === selectedSize);
  ...
  return (
    <Box>
      <TopHeader />
      <Header />
      <Divider sx={{ mt: 2, width: '81%', marginLeft: '170px' }} />
      <Container maxWidth="xl" sx={{ marginTop: '50px' }}>
        <Grid container spacing={4}>
          ...
          <Grid item xs={12} md={6}>
            ...
            <Typography
              variant="body1"
              sx={{ mt: 2, fontSize: '16px', lineHeight: '22px', fontWeight: '400', color: 'grey',width:'550px',height:'33px' }}
            >
